...
the IoT Agent, the device will send, as part of a normal measure, the query parameter 'getCmd' with value '1'. This parameter
can be send as part of a normal measure or in a request without any measure (so the device is not forced to send a 
measure in order to get the list of commands). As a result of this action, the IoTAgent, instead of returning an empty body 
(the typical response to a measurement report), it will return a list of all the commands available for the device, 
sepparated by the character '#'. The command payload is described in the protocol section (and its shared with the push
