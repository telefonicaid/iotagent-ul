...
function createCommandPayload(device, command, attributes) {
    function addAttributes(current, key) {
        let cmd = device && device.commands.find((att) => att.name === current);
        ...
        if (cmd && cmd.expression) {
            ...
            return device.id + '@' + current + '|' + value;
        } else {
            return device.id + '@' + current + '|' + key + '=' + value;
        }
    }
    if (typeof attributes === 'object') {
        return Object.keys(attributes).reduce(addAttributes, command);
    }
