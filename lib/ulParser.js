    ...
    if (command && command.payloadType) {
        switch (command.payloadType.toLowerCase()) {
            case 'binaryfromstring':
                ...
                break;
            case 'binaryfromhexstring':
                serialized = Buffer.from(payload.toString('hex'));
                break;
            default:
                serialized = payload;
        }
...
function createCommandPayload(device, command, attributes) {
    function addAttributes(current, key) {
        //let cmd = device && device.commands.find((att) => att.name === key);
        let value = attributes[key];
        // if (cmd && cmd.expression) {
        //     let parser = iotAgentLib.dataPlugins.expressionTransformation;
        //     let ctxt = device.staticAttributes.concat({key: value});
        //     value = parser.applyExpression(cmd.expression, ctxt, device);
        // }
    ...
    if (cmd && cmd.expression) {
        let parser = iotAgentLib.dataPlugins.expressionTransformation;
        let ctxt = device.staticAttributes.concat({ command: attributes });
        attributes = parser.applyExpression(cmd.expression, ctxt, device);
    }
