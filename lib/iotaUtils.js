...
function findOrCreate(deviceId, transport, group, callback) {
    iotAgentLib.getDeviceSilently(deviceId, group.service, group.subservice, function (error, device) {
        if (!error && device) {
            ...
            if (newDevice.autoprovision === undefined || newDevice.autoprovision === true) {
                ...
                config
                    .getLogger()
                    .info(context, 'Device %j not provisioned due autoprovision is disabled by its conf', newDevice);
                callback(null, newDevice, group);
            }
        } else {
            callback(error);
        }
...
function mergeDeviceWithConfiguration(deviceData, configuration, callback) {
    ...
    if (configuration && configuration.autoprovision !== undefined && deviceData.autoprovision === undefined) {
        deviceData.autoprovision = configuration.autoprovision;
    }
