...
function generateCommandExecution(apiKey, device, attribute) {
    ...
    var commandTopic;
    if ( ('commandLegacy' in device && device.commandLegacy !== undefined) ? 
         device.commandLegacy : config.getConfig().mqtt.commandLegacy === true ) {
        commandTopic = '/' + apiKey + '/' + device.id + '/cmd';
    } else {
        commandTopic = constants.MQTT_TOPIC_PROTOCOL + '/' + apiKey + '/' + device.id + '/cmd';
    }
...
function sendConfigurationToDevice(apiKey, deviceId, results, callback) {
    ...
    var commandTopic;
    if (
        /*'commandLegacy' in device && device.commandLegacy !== undefined
           ? device.commandLegacy :*/
        config.getConfig().mqtt.commandLegacy === true
    ) {
        commandTopic =
            '/' +
            apiKey +
