...
function updateCommand(apiKey, deviceId, device, message) {
        ...
        if (error) {
            ...
            if (topicInformation[4]) {
                ...
                if (topicInformation[3] === constants.CONFIGURATION_COMMAND_SUFIX) {
                    updateCommand(apiKey, deviceId, device, message.toString());
                } else if (topicInformation[3] === constants.MEASURES_SUFIX) {
                    multipleMeasures(apiKey, deviceId, device, message.toString());
                } else {
 ...
 *
 * @param {String} id           Name of the entity affected by the command request.
 * @param {String} type         Type of entity affected by the commit.
...
function stop(callback) {
    logger.info('Stopping MQTT Binding' + (new Date()));

    async.series([
        unsubscribeAll,
        mqttClient.end.bind(mqttClient, true)
    ], function() {
        logger.info('MQTT Binding Stopped' + (new Date()));
        callback();
    });
}
