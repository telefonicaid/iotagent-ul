...
function generateCommandExecution(apiKey, device, attribute) {
    ...
    return function sendUlCommandHTTP(callback) {
        ...
        if (config.getConfig().http.timeout) {
            options.timeout = {
                request: config.getConfig().http.timeout
            };
        }

        got.post(device.endpoint, options)
            .then((response) => {
                ...
                if (response && response.statusCode !== 200) {
                    ...
                    return callback(new errors.HTTPCommandResponseError(response.statusCode, errorMsg, cmdName));
                } else if (apiKey) {
                    commandObj = ulParser.result(body);
            ...
            .catch((error) => {
                return callback(new errors.HTTPCommandResponseError('', error, cmdName));
            });
    };
}
