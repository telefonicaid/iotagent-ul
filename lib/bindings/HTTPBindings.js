...
/**
 * Get correlator for a given request if fiware-correlator header is present or generates
 * a random one otherwise.
 *
 * @param req request
 * @return correlator string
 */
function getCorr(req) {
    if (req.get(CORRELATOR_HEADER)) {
        return req.get(CORRELATOR_HEADER);
    } else {
        return uuid.v4();
    }
}
...
/**
 * Set logger to be used for logging in the context of the HTTP request, with the
 * proper correlator.
 *
 * @parm req  request
 * @param res  response
 * @param next  callback to invoke upon finalization
 */
function setRequestLogger(req, res, next) {
    var corr = getCorr(req);
    req.logger = logops.child({ corr: corr, trans: corr });
    next();
}
...
function handleError(error, req, res, next) {
    ...
    var logger = logops.child({ corr: corr, trans: corr });
...
function parseData(req, res, next) {
    ...
    req.logger.debug(context, 'Parsing payload [%s]', payload);
    ...
    if (error) {
        ...
        req.logger.debug(context, 'Parsed data: [%j]', data);
        next();
    }
}
...
function handleIncomingMeasure(req, res, next) {
    ...
    req.logger.debug('Processing multiple HTTP measures for device [%s] with apiKey [%j]', req.deviceId, req.apiKey);
    ...
    utils.getEffectiveApiKey(device.service, device.subservice, function(error, apiKey) {
        async.series(attributes.map(generateCommandExecution.bind(null, apiKey, device)), function(error) {
            if (error) {
                config
                    .getLogger()
                    .error(context, 'COMMANDS-004: Error handling incoming command for device [%s]', device.id);
