...
function handleIncomingMeasure(req, res, next) {
    ...
    function findOrCreate(group, callback) {
        iotAgentLib.getDevice(req.deviceId, group.service, group.subservice, function(error, device) {
            if (!error && device) {
    ...
    if (req.apiKey === config.getConfig().defaultKey) {
