...
function processMeasureGroup(device, apikey, previous, current, index) {
    ...
    if (current.command) {
        ...
        for (const k in current) {
            if (current !== undefined || current === Object || Buffer.isBuffer(current)){
                var arr = [];
                for (var i = 0; i < current[k].length; i++) {
                    arr[i] = current[k].charCodeAt(i).toString(16);
                }
                current[k] = arr.join('');
            }
...
function multipleMeasures(apiKey, device, messageStr) {
    ...
    // Parse Message
    try {
        messageStr = messageStr.replace(/0x|,/g, '');
