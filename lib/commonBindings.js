...
function messageHandler(topic, message, protocol) {
    ...
    if (['json', 'ul'].includes(topicInformation[1].toLowerCase())) {
        topicProtocol = topicInformation[1].toLowerCase();
        apiKey = topicInformation[2];
        deviceId = topicInformation[3];
    } else {
        topicProtocol = 'ul';
    ...
    if (topicProtocol !== 'ul') {
        config.getLogger().debug(context, 'Topic protocol [%s] not expected by this agent ', topicProtocol);
        return;
    }
    messageStr = message.toString();
