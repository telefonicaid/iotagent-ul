 ...
 * @param {String} apiKey           API Key corresponding to the Devices configuration.
 * @param {Object} device           Device object containing all the information about a device.
 * @param {String} messageStr       String UL payload.
 * @param {String} message          UL payload.
 */
function multipleMeasures(apiKey, device, messageStr, message) {
    let updates = [];
    let parsedMessage;
    ...
    try {
        ...
        config
            .getLogger()
            .debug(context, 'Parse error parsing incoming message [%]. Forcing to hex string', messageStr);
 ...
 * @param {Object} device           Device object containing all the information about a device.
 * @param {Buffer} message          String Raw message coming from the client.
...
function messageHandler(topic, message, protocol) {
    ...
    function processMessageForDevice(device, apiKey, topicInformation) {
        ...
        if (
            ...
            multipleMeasures(apiKey, device, messageStr, message);
        } else {
            config.getLogger().error(
                context,
                /*jshint quotmark: double */
